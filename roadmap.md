...
19. [in-progress] [Roadmap] {GPT-5 Codex}: Modal stream V2 planning and tuning pass: preserve the agreed next-step product direction and then implement it in phases. Why it matters: the prototype works, but the next iteration now has important product rules that are easy to lose in chat alone, including stub-card priority, animated slide transitions with tunable easing, future chunked/unit paging, and a top entry-composition panel with span-level override semantics. Progress: Phases 1-4 are now implemented through `v0.3.7-alpha` (stub packing, motion, display composition panel, and field-level manual overrides). Suggested next step: complete Phase 5 chunked/unit paging using `operations/plans/PLAN-modal-stream-v2-stubs-motion-composition.md` as the source of truth.
