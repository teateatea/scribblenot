...
## Now
**A** `assigned-item-label-token-resolution` — resolve `{label}` tokens for assigned items before modal display and note output (#38) — highest-priority remaining correctness follow-up now that empty optional modal confirmation is fixed
**B** `semantic-stub-authority` — make modal stub meaning come from real modal-edge semantics and keep the unresolved active-row width follow-through in the same modal/UI lane (#28, #33) — safest parallel branch beside A because it stays mostly in modal/layout semantics
**C** `section-body-schema` — add explicit authored section `body:` typing and tighten remaining loader reference validation while that schema surface is open (#22, #3) — safest third branch beside A and B; `#29` should wait until the schema and modal semantics settle because header/map hints still share one navigation stream

## Next
38. [open] [Reliability] [I: 60, D: 45] {Claude Sonnet 4.6}: Assigned items do not resolve {label} tokens when displayed in modal lists — e.g. selecting "2" renders as "week{pluralizer}" instead of "weeks". Why it matters: unresolved tokens appear verbatim in the modal list and in note output, producing malformed text that undermines the value of the format system. Suggested next step: find where assigned-item labels are rendered in the modal list and ensure the same token-resolution pass applied to output strings is also applied to display labels before they are shown. Status note: subjective preview/export placeholder leakage from empty assigned format-list values was fixed under #37; the remaining open scope here is the modal-row label surface itself.
28. [open] [Architecture] [I: 55, D: 65] {GPT-5 Codex}: Establish semantic stub authority for modal edges. Why it matters: repeat-joiner teaser coverage has improved, but stub meaning is still ultimately inferred from preview/layout boundaries rather than real modal behavior, which will keep causing correctness drift as branch, nested, filtered, and other less-previewable flows get more complex. Suggested next step: implement `operations/plans/PLAN-modal-stub-option-2-semantic-authority.md` so `>`, `+`, and `-` come from explicit edge semantics in both still rendering and frozen transition geometry, not teaser availability alone.
...
29. [planned] [Features] [I: 50, D: 60] {GPT-5 Codex}: Add explicit single-character `hotkey:` support for hinted entities so authored sections, header fields, and item-backed modal rows can reserve mnemonic labels instead of relying only on generated hints. Why it matters: the current hint system is fully generated and some hint surfaces already share one live resolution path, which makes high-frequency actions harder to memorize and makes authored collisions easy to mishandle unless scope rules are explicit. Suggested next step: implement `operations/plans/PLAN-explicit-hotkeys-for-sections-fields-items.md`, but validate conflicts across the full simultaneously-resolvable scope (especially section-vs-field hints in header mode), keep collection-name rows out of v1, and preserve shorthand-item limits plus text-entry precedence. Status note: plan review on 2026-04-20 found that header-mode section hints and field hints are both active today, so duplicate checks cannot stop at per-entity-kind validation.
