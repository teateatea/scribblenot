...
## Now
**A** `optional-modal-empty-confirm` — let sections complete when an optional modal confirms an intentional empty-string value (#37) — highest-priority correctness follow-up because it blocks valid note completion in real flows
**B** `semantic-stub-authority` — make modal stub meaning come from real modal-edge semantics and keep the unresolved active-row width follow-through in the same modal/UI lane (#28, #33) — safest parallel branch beside A because it stays mostly in modal/layout semantics; `#38` is higher in raw priority but likely overlaps A's modal rendering/output lane
...
## Next
37. [open] [Reliability] [I: 65, D: 40] {Claude Sonnet 4.6}: Subjective entries incorrectly refuse to complete when a modal has a confirmed empty-space entry. Why it matters: some modals are optional and will regularly produce a "" output; blocking full confirmation on an empty confirmed value prevents valid note completion. Suggested next step: audit the confirm-to-note path for any checks that gate on modal output content, and ensure empty-string confirmed entries from optional modals are passed through rather than treated as incomplete.
38. [open] [Reliability] [I: 60, D: 45] {Claude Sonnet 4.6}: Assigned items do not resolve {label} tokens when displayed in modal lists — e.g. selecting "2" renders as "week{pluralizer}" instead of "weeks". Why it matters: unresolved tokens appear verbatim in the modal list and in note output, producing malformed text that undermines the value of the format system. Suggested next step: find where assigned-item labels are rendered in the modal list and ensure the same token-resolution pass applied to output strings is also applied to display labels before they are shown.
