...
## Now
**A** `test-safety` — test infrastructure quality and clipboard import safety (#17, #5, #20) — branch open
**B** `modal-correctness` — confirmed-slot scoping and modal visual corrections; safe to open alongside A (#31, #27, #32, #35) — branch open; `#33` is deferred with an investigation note attached, and #23 is next by priority but conflicts with #31 on `src/app.rs`
**C** `schema-anchors` — stable document anchors and loader schema tightening; safe to open alongside A and B (#15, #3, #22) — #23 remains blocked until B closes

## Next
17. [planned] [Security] [I: 75, D: 40] {GPT-5 Codex}: Add explicit regression checks for clipboard-import config-persistence boundaries. The planned clinic-note import flow is intentionally memory-only, and leaking patient text into persisted user config would be a high-impact failure. Suggested next step: implement `operations/plans/PLAN-17-clipboard-import-privacy-regression.md`, which adds a temp-dir-backed config-save harness in `src/app.rs` and creates `src/import.rs` with a stub `try_parse_clipboard_note` plus narrow regression tests that guard the current config-save paths before and after Phase 2 import is built.
31. [in-progress] [Reliability] [I: 70, D: 65] {GPT-5 Codex}: Scope item `assigns` per confirmed field slot instead of flattening them into one global map. Why it matters: the narrow stale-state fixes make current modal editing safer, but confirmed rendering still merges assigned format-list outputs across fields and repeats, so one slot can silently overwrite another. Suggested next step: implement `operations/plans/PLAN-assignment-slot-scoping-regression.md` (structural fix already landed; regression coverage is the remaining work). Current branch: `modal-correctness`. Note: `PLAN-assignment-slot-scoping.md` is superseded.
15. [planned] [Architecture] [I: 70, D: 55] {GPT-5 Codex}: Define stable per-section editable-document anchors before completing the desktop rewrite. The tray app needs safe section-level replacement inside `editable_note`, but current note rendering groups multiple sections under shared headings. Suggested next step: make the anchor contract explicit in the rewrite docs and keep document helpers keyed to stable per-section anchors, not only top-level headings. Plan: `operations/plans/section-anchor-contract.md`.
...
5. [planned] [Tooling] [I: 60, D: 50] {GPT-5 Codex}: Add task-focused integration tests for current user-editable flows, especially composition/manual-override note preview generation and editable-document sync. The unit suite is healthy, but these cross-state behaviors are where regressions are still likely to slip through. Plan: `operations/plans/composition-sync-tests.md`.
27. [in-progress] [UI] [I: 60, D: 50] {GPT-5 Codex}: Modal stub preview-sequence fix for repeat-joiner lists. Why it matters: the current modal stream can show a terminal green `+` on non-terminal states like `obmuscle_field`'s `muscle` list because teaser generation stops early for repeat-joiner lists even when real field flow continues. Suggested next step: implement `operations/plans/PLAN-modal-stub-option-1-preview-sequence-fix.md` and add regression coverage for repeat lists with downstream modal states. Current branch: `modal-correctness`.
...
35. [in-progress] [UI] [I: 50, D: 35] {GPT-5 Codex}: Show the confirm border-box style on the item before confirm-triggered transitions begin in the final modal. Why it matters: confirming an item in either final-modal path currently starts the unit-transition or confirm-close motion immediately, so the row never visibly enters its confirmed styling during the transition and ends up looking too similar to a plain `nav_right` focus move where no confirmation happened. Suggested next step: make confirm actions apply the confirmed-row visual state for at least the first rendered transition frame, then add a focused regression or screenshot check that distinguishes confirm from `nav_right` in both final-modal paths. Current branch: `modal-correctness`.
20. [planned] [Tooling] [I: 45, D: 25] {GPT-5 Codex}: Stop tests from writing `config.yml` into the repo root. Why it matters: the current App tests can leave runtime artifacts in the worktree, which is noisy and increases the risk of accidentally staging local config. Suggested next step: route config persistence through a temp/test data dir in App tests or make config writes injectable/no-op under targeted test setups. Plan: `operations/plans/20-config-save-empty-guard.md`.
