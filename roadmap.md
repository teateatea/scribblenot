...
## Now
**A** `runtime-node-navigation` — key map/wizard state and preview grouping off runtime node IDs instead of section-count math (#23) — highest-priority follow-up; keep `#21` separate until the node-ID cutover settles
**B** `semantic-stub-authority` — make modal stub meaning come from real modal-edge semantics and keep the unresolved active-row width follow-through in the same modal/UI lane (#28, #33) — safe beside A because it stays mostly in modal/layout code
**C** `section-body-schema` — add explicit authored section `body:` typing and tighten remaining loader reference validation while that schema surface is open (#22, #3) — safest third branch beside A and B; `#29` should wait until A settles because header/map hints still share one navigation stream, and it also overlaps B's modal row labeling
