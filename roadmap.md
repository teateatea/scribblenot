...
## Now
**A** `semantic-stub-authority` — make modal stub meaning come from real modal-edge semantics and keep the unresolved active-row width follow-through in the same modal/UI lane (#28, #33) — safest parallel branch now that assigned-item token resolution is closed
**B** `section-body-schema` — add explicit authored section `body:` typing and tighten remaining loader reference validation while that schema surface is open (#22, #3) — safest second branch beside A; `#29` should wait until the schema and modal semantics settle because header/map hints still share one navigation stream
