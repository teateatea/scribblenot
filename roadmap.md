...
## Now
**A** `semantic-stub-authority` — make modal stub meaning come from real modal-edge semantics and keep the unresolved active-row width follow-through in the same modal/UI lane (#28, #33) — highest-priority follow-up now that the node-ID/runtime-tree cutover has landed
**B** `section-body-schema` — add explicit authored section `body:` typing and tighten remaining loader reference validation while that schema surface is open (#22, #3) — safe beside A because it stays in loader/schema/runtime dispatch work
**C** `reload-checksum` — avoid redundant modal-unit rebuilds on no-op reloads now that the runtime-node cutover has settled (#21) — safe third branch because it is focused on reload/cache plumbing rather than modal semantics or schema work
